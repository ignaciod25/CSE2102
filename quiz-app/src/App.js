...
const App = () => {
  ...
  const [timerMinutes, setTimerMinutes] = useState(2); // Set the initial timer minutes
  const [timerSeconds, setTimerSeconds] = useState(0); // Set the initial timer seconds
    ...
    if (quizStarted && !quizComplete) {
      interval = setInterval(() => {
        if (timerMinutes === 0 && timerSeconds === 0) {
          handleNextClick(); // Automatically move to the next question when the timer reaches 0
        } else {
          if (timerSeconds === 0) {
  ...
  const handleTryAgainClick = () => {
    ...
    setTimerMinutes(2); // Reset the timer for the new quiz 
    setTimerSeconds(0);
  };
